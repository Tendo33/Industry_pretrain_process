...
def merge_json_files_to_jsonl(root_dir):
    # 遍历根目录下的所有文件夹
                    ...
                    if json_filename.endswith('.json'):
                        with open(json_filepath, 'r', encoding='utf-8') as json_file:
                            try:
                                ...
                                print(
                                    f"Error decoding JSON from file: {json_filepath}")

            print(
                f"All JSON files from {folder_path} have been merged into {jsonl_filepath}")


# 使用示例
root_directory = r"/data/nfs/data/IndustryCorpus/chinese"
merge_json_files_to_jsonl(root_directory)
