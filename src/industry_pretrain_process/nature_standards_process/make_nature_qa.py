...
## Workflow
1. 接收用户输入的段落。
2. 分析段落内容，提出若干个相关的问题,问题应该表述详细，并且使用严谨的描述风格。
3. 从段落中找到并给出相应的详细并严谨的回答。
4. 按照指定的格式输出结果。
"""
...
def make_prompt(title: str, text: str) -> str:
    prompt = f"""
    这是{title}文件中的一个段落chunk，根据下面的段落文字提取出若干个中文的详细并严谨的问题对，名称主语完整无误，不能使用类似“本文件，表2，表A”这类似笼统以及局限的代词。：
