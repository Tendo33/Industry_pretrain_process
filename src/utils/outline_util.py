...
def is_increasing(numerical_list: list[int]) -> bool:
    ...
    for i in range(len(numerical_list) - 1):
        # 判断数字序列是否递增
        if numerical_list[i+1] - numerical_list[i] != 1 and numerical_list[i+1] != 1:
            return False
    return True


# 筛选出符合大纲的正文,返回正文和标题
def extract_outlines_files(content_list: list[str], title_list: list[str]) -> tuple[list[str], list[str]]:
    # 存储清洗后的内容列表
...
def extract_outlines_new(content_list: list[str]) -> list[str]:
    ...
    for i, content in enumerate(content_list):
        ...
        output_dagang = [element.strip() for element in content if level_two_pattern.match(
            element) or level_thr_pattern.search(element)]
        output_dagang = [re.split('。|：|，|；', element)
                         for element in output_dagang]
        output_dagang = [
            part for element in output_dagang for part in element if part]
        # 去除所有文字，只保留大纲
        output_dagang = [element for element in output_dagang if level_two_pattern.match(
            element) or level_thr_pattern.search(element)]
...
def extract_policy_files(content_list: list[str], title_list: list[str]) -> list[str]:
